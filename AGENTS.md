...
- `--help` に表示されるサブコマンド・オプション・フラグの説明文は英語で書くこと
  - `noargs::cmd().doc()` / `noargs::opt().doc()` / `noargs::flag().doc()` のすべてが対象
...
- Snapshot テスト (`tests/sqlc.rs`): SQL → コード生成の出力全体を insta でスナップショット検証する
...
- 依存ライブラリは勝手に追加しないこと
- JSON ライブラリは nojson を使うこと
- CLI ライブラリは noargs を使うこと
- ロギングライブラリは tracing を使うこと
- 暗号ライブラリは aws-lc-rs を使うこと
- TLS ライブラリは rustls を使うこと
- 非同期ライブラリは tokio を使うこと
- HTTP ライブラリは shiguredo_http11 を使うこと
